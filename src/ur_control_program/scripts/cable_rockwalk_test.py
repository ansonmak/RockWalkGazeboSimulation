...
from gazebo_msgs.srv import SpawnModel, DeleteModel, SetModelState, ApplyBodyWrench, BodyRequest, SetLinkState
import os #print without new line, use ~/ for path
from gazebo_msgs.msg import ContactsState, ModelState, LinkState
...
class Ur10RoboticArm:
  ...
  def initialize_arm_pose(self):
    ...
    initial_joint_value = []
    initial_joint_value.append(1.4016569197)
    initial_joint_value.append(-0.8013272686)
    initial_joint_value.append(1.2040469962)
    initial_joint_value.append(-0.4026055949)
    initial_joint_value.append(1.4052019697)
    initial_joint_value.append(0.0022420732)
...
class Oblique_cone:
  def __init__(self):
    ...
    self.rod_z = 0.75
    self.rod_roll = 0
    self.ring_radius = 0.02
    #spawn position
    self.spawn_pose = Pose()
    self.spawn_pose.position.x = 0
    self.spawn_pose.position.y = 1.8
    self.spawn_pose.position.z = 0
    #state info
    self.model_state = ModelState()
    self.model_state.model_name =  "cable_cone"
    self.link_state = LinkState()
    self.link_state.link_name = "cable_cone::ring"
  ...
  def spawn_model(self):
    ...
    #spawn model
    self.spawn("cable_cone",self.cone_xml,"",self.spawn_pose,"world")
    print "Cone model spawned"

  def delete_model(self):
    print_1line("Waiting for gazebo delete model service...")
    rospy.wait_for_service('/gazebo/delete_model')
    self.delete = rospy.ServiceProxy('/gazebo/delete_model', DeleteModel) 
    self.delete("cable_cone")
    print "Cone model deleted"

  def subscribe_imu(self):
    print_1line("Subscribing to oblique cone imu topic...")
    self.imu_subscriber = rospy.Subscriber('/cable_cone/imu', Imu, self.imu_handler)
    print "Done"
  ...
  def subscribe_bumper(self):
    ...
    self.bumper_subscriber = rospy.Subscriber('/cone_bumper', ContactsState, self.bumper_handler)
    print "Done"
  ...
  def init_set_model_state_service(self):
  ...
  def set_cone_pose(self, target_pose):
    self.model_state.pose = target_pose
    self.set_model_state(self.model_state)

  def reset(self):
    self.set_cone_pose(self.spawn_pose)
  ...
  def init_set_link_state_service(self):
    print_1line("Waiting for gazebo set link state service...")
    rospy.wait_for_service('/gazebo/set_link_state')
    #setup set state function
    self.set_link_state = rospy.ServiceProxy('/gazebo/set_link_state', SetLinkState)
    print "Done"

  def set_ring_pose(self, target_pose):
    self.link_state.pose = target_pose
    self.set_link_state(self.link_state)
  ...
  def excite_cone_motion(self, excitation_angle):
    ...
    self.apply_wrench("cable_cone::experimental_cone::cone", "world", Point(), wrench, rospy.Time(), rospy.Duration(-1))
    ...
    self.apply_wrench("cable_cone::experimental_cone::cone", "world", Point(), wrench, rospy.Time(), rospy.Duration(-1))
    self.clear_wrench("cable_cone::experimental_cone::cone")
...
class Rocking_Walking_Manipulation:
  ...
  def manipulate_apex_position(self, ur10, cone):
    ...
    while step <= self.step and not rospy.is_shutdown():
      if abs(cone.imu.angular_velocity.z) < 0.3:
...
def main():
  ...
  # cone.init_set_model_state_service()
  # cone.reset() #reset cone position
  # cone.init_set_link_state_service()
